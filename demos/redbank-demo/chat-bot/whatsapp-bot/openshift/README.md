...
If you're deploying GPU-requiring workloads (e.g., Whisper model serving), you need the NVIDIA GPU Operator installed:

```bash
# Install NVIDIA GPU Operator
./openshift/install-nvidia-operator.sh

# Or see troubleshooting guide for manual installation
# See: openshift/NVIDIA_GPU_OPERATOR_TROUBLESHOOTING.md
```
...
4. **GPU pods failing to schedule**: See [NVIDIA GPU Operator Troubleshooting](NVIDIA_GPU_OPERATOR_TROUBLESHOOTING.md)

### GPU-Related Issues
If you see errors like "Insufficient nvidia.com/gpu" when deploying GPU workloads:

```bash
# Quick fix: Install NVIDIA GPU Operator
./openshift/install-nvidia-operator.sh
...
oc get clusterpolicy -n openshift-operators
```

For detailed GPU troubleshooting, see: [NVIDIA_GPU_OPERATOR_TROUBLESHOOTING.md](NVIDIA_GPU_OPERATOR_TROUBLESHOOTING.md)
